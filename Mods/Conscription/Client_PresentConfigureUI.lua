...
function Create_UI_Controls(rootParent)
    ...
    UI.CreateLabel(modBehaviourVGroup).SetText('Bonus Value Decrease (permanent):');
    ...
    UI.CreateLabel(modBehaviourVGroup)
    .SetText('Bonuses that start at 0 value are excluded*')
    .SetColor(BUTTON_COLOURS.DarkGray);

    UI.CreateLabel(modBehaviourVGroup)
    .SetText('If 1 Army Must Stand Guard is enabled, values of 1 neutralise*')
    .SetColor(BUTTON_COLOURS.DarkGray);

    local neutraliseSubOptionsParent = UI.CreateVerticalLayoutGroup(modBehaviourVGroup);
...
function Create_Neutralise_SubOptions_UI(rootParent)
    ...
    UI.CreateLabel(neutraliseSubOptionsVGroup)
    .SetText('At least 1 applicable territory always neutralises per turn as long as this is at least 1%*')
    .SetColor(BUTTON_COLOURS.DarkGray);
end
