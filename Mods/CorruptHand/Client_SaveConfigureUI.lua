...
function Client_SaveConfigureUI(alert, addCard)
    Mod.Settings.CorruptHandGivesCorruptionImmediately = corruptHandGivesCorruption.GetIsChecked();
    if (Mod.Settings.CorruptHandGivesCorruptionImmediately) then
        Mod.Settings.TurnsUntilCorruption = 0;
    else
        Mod.Settings.TurnsUntilCorruption = turnsUntilCorruption.GetValue();
        if (Mod.Settings.TurnsUntilCorruption < 1) then
            alert("Turns until Budding Corruption matures cannot be less than 1");
            return;
        end
    end
    ...
    Mod.Settings.BuddingCorruptionDifficulty = buddingCorruptionDifficulty;
    ...
    local buddingCorruptionCardID = addCard(
        ...
        Mod.Settings.BuddingCorruptionDifficulty .. "BuddingCorruption.png",
        1, 0, 0, 0);
