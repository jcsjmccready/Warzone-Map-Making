...
UI2 = {
    Colors = {
        ...
        HelpButtonColor = "#50B2E3";

        SecondaryButton = "#242D9A";
...
---     for i = optionsPerPage * (pageNumber - 1) + 1, optionsPerPage * pageNumber do
---         -- show option
---     end
--- 
...
function UI2.CreatePageButtons(parent, pageN, maxPage, prevFunc, nextFunc, color)
    color = color or UI2.Colors.RoyalBlue;
    ...
    UI2.CreateLabel(line).SetText(pageN .. " / " .. maxPage).SetColor(UI2.Colors.TextDefault);
    UI2.CreateEmpty(line).SetPreferredWidth(5);
...
---     function(line)
---         label = CreateLabel(line).SetText("Clicking the '-->' to go to the next page");
---     end, 
---     function()
---         UI2.Destroy(label);  -- 'label' is a global variable
---     end, 
---     "-->", 
---     UI2.Colors.Yellow);       -- color Red
---```
function UI2.CreateActionButtonLine(parent, func, action, actionText, actionButtonColor)
    ...
    UI2.CreateButton(line).SetText(actionText or "?").SetColor(actionButtonColor or UI2.Colors.HelpButtonColor).SetOnClick(action);
end

---Creates an info action button, with text '?' and the same color as the guide buttons. When the user interacts with this button, it will pop up any alert dialog
---@param parent UIObject # The parent
...
function UI2.CreateInfoButtonLine(parent, func, text)
    UI2.CreateActionButtonLine(parent, func, function() UI2.Alert(text); end, "?", UI2.Colors.HelpButtonColor);
end
...
function UI2.CreateDeleteButtonLine(parent, func, action)
    UI2.CreateActionButtonLine(parent, func, action, "X", UI2.Colors.DeleteButton);
end
...
---Registers a new dialog. If a dialog with the same name is still open, it will close that dialog
---@param id string | number # Unique identifier of the dialog. Best use case is to give all dialogs a distinguishable name
---@param rootParent RootParent # The RootParent of the dialog. This is used to check whether the dialog is still open or not
---@param close fun() # The close() function of the dialog. Needed to close the dialog
---```
...
---@param id string | number # The identifier of the dialog. If the dialog was never registered, or was already closed, this call will do nothing
---```
...
---Closes all dialogs that were registered with the NewDialog function
---```
...
---Returns true if the passed identifier is a dialog and is open
---@param id string|number
---@return boolean
---```
--- if UI2.DialogIsOpen("Some_ID") then
---     print("Dialog 'Some_ID' is still open!");
--- end
---```
function UI2.DialogIsOpen(id)
    return Dialogs_JAD[id] and not UI2.IsDestroyed(Dialogs_JAD[id].RootParent);
end

---Creates and returns a vertical layout group. Same as calling CreateVerticalLayoutGroup
---@param parent UIObject # The parent
...
---@field SubmitButton "#198225"
---@field SaveButton "#198225"
---@field PrimaryButton "#198225"
---@field CancelButton "#9A2929"
---@field DeleteButton "#9A2929"
---@field BuildButton "#ABA500"
---@field HelpButtonColor "#50B2E3"
---@field SecondaryButton "#242D9A"
