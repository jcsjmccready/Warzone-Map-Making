...
function Server_AdvanceTurn_Order(game, order, result, skipThisOrder, addNewOrder)
    ...
    -- an order made from this mod's menu, it asks this mod to send a test order to the other mod
    if (order.proxyType == 'GameOrderCustom' and order.Payload == SEND_ORDER_PAYLOAD) then
...
---Called for each order the other mod has authenticated with this mod, it adds an order saying it arrived
...
function OnAuthenticatedOrderReceived(senderMod, data, order, addNewOrder)
    -- only orders from the mod this one is being tested with
    if (senderMod ~= OTHER_MOD_KEY) then return; end
