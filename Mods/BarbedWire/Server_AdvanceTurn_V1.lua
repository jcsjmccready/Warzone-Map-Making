...
--Settings version 1 behaviour (games saved before Mod.Settings.Version existed)

---@class V1_PendingBarbedWire
---@field PlayerID PlayerID
---@field Message string
---@field TerritoryID TerritoryID

---@class V1_PrivateGameData
---@field PendingBarbedWire V1_PendingBarbedWire[] 
---@field TriggeredTerritoryIds TerritoryID[]
...
function V1.Server_AdvanceTurn_Order(game, order, result, skipThisOrder, addNewOrder)
    ...
    if (order.proxyType == 'GameOrderPlayCardCustom' and startsWith(order.ModData, "CreateBarbedWire_")) then
		...
		---@type V1_PendingBarbedWire
		local pendingBarbedWire = {
			PlayerID = order.PlayerID,
			Message = (order --[[@as GameOrderPlayCardCustom]]).Description, -- Description only exists on the custom-card subtype, matched by proxyType above
			TerritoryID = targetTerritoryID --[[@as TerritoryID]], -- always numeric here, having matched the "CreateBarbedWire_" prefix above
		};

		local privateGameData = Mod.PrivateGameData --[[@as V1_PrivateGameData]];
	...
	V1.HandleAttackTransferInTriggeredBarbedWire(game, order, result, skipThisOrder, addNewOrder);
	V1.HandleAttackTransferToBarbedWire(game, order, result, addNewOrder);
end

---@param game GameServerHook
---@param addNewOrder fun(order: GameOrder) # Adds a game order, will be processed before any of the rest of the orders
function V1.Server_AdvanceTurn_End(game, addNewOrder)
	V1.BuildStructures(game, addNewOrder);
	V1.ResetTriggeredBarbedWire(game, addNewOrder);
end
...
---@param skipThisOrder fun(modOrderControl: EnumModOrderControl) # Allows you to skip the current order
---@param addNewOrder fun(order: GameOrder) # Adds a game order, will be processed before any of the rest of the orders
function V1.HandleAttackTransferInTriggeredBarbedWire(game, order, result, skipThisOrder, addNewOrder)
...
---@param result GameOrderResult
---@param addNewOrder fun(order: GameOrder) # Adds a game order, will be processed before any of the rest of the orders
function V1.HandleAttackTransferToBarbedWire(game, order, result, addNewOrder)
...
---@param game GameServerHook
---@param addNewOrder fun(order: GameOrder) # Adds a game order, will be processed before any of the rest of the orders
function V1.ResetTriggeredBarbedWire(game, addNewOrder)
	local triggeredBarbedWireStructId = WL.StructureType.Custom("TriggeredBarbedWire");
	local primedBarbedWireStructId = WL.StructureType.Custom("PrimedBarbedWire");
...
---@param game GameServerHook
---@param addNewOrder fun(order: GameOrder) # Adds a game order, will be processed before any of the rest of the orders
function V1.BuildStructures(game, addNewOrder)
